...
export async function sendToBackend(errorData) {
  try {
    ...
    const backendUrl = settings.backendUrl || 'https://api.consolewhisperer.com';
    ...
    return {
      ...
      explanation: "We couldn't connect to our AI service to explain this error. Please check your internet connection and try again.",
      suggestedFix: null,
      links: [],
      offline: true
    };
...
export async function getLinks(errorMessage) {
  try {
    ...
    // Send the request
    const response = await fetch(`${backendUrl}/api/links?query=${encodeURIComponent(errorMessage)}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
